 ...
 * @since 1.0.0
 */
if (!class_exists('newsx_paper_lite_About')) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class newsx_paper_lite_About
	{
		/**
		 ...
		 *
		 * @var newsx_paper_lite_About $instance The  TI_About_Page instance.
		 */
		...
		{
			if (!isset(self::$instance) && !(self::$instance instanceof newsx_paper_lite_About)) {
				self::$instance = new newsx_paper_lite_About;
		...
		public function free_pro()
		{
			...
			if (!empty($free_pro)) {
				if (!empty($free_pro['free_theme_name']) && !empty($free_pro['pro_theme_name']) && !empty($free_pro['features']) && is_array($free_pro['features'])) {
					...
					echo esc_html('NewsX Paper Pro Live Preview', 'newsx-paper-lite');
					echo '</a></td></tr>';
			...
			if ('appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix) {
				...
				wp_localize_script(
					...
					array(
						...
						'activating_string'        => __('Activating', 'newsx-paper-lite')
					)
				);
			}
		}
